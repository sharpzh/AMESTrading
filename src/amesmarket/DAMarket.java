...
/*    
    ...
    String strTemp=String.format("%1$10s\t%2$15s\t%3$15s\t%4$15s\t%5$15s\t%6$15s\t%7$15s\t%8$15s","GenCo","A","B","CapMin","CapMax","emcoe","EA");
    System.out.println(strTemp);
    double dGenMinCapacity=0.0;
    double dGenMaxCapacity=0.0;
    double dGenMAXEA=0.0;
    for(int i = 0; i<trueSupplyOfferByGen.length; i++){
        dGenMinCapacity+=trueSupplyOfferByGen[i][3];
        dGenMaxCapacity+=trueSupplyOfferByGen[i][4];
        dGenMaxEA+=trueSupplyOfferByGen[i][6];
    ...
    System.out.println("The true max total gen Emission Allowance is: "+dGenMaxEA);
    System.out.println();
*/
  ...
  public void dayAheadOperation(int h, int d){
    ...
    //System.out.println("\nHour " +  h + " Day " + d +  ": Day-Ahead Market operation.");
      ...
      String strTemp=String.format("%1$10s\t%2$15s\t%3$15s\t%4$15s\t%5$15s\t%6$15s\t%7$15s","GenCo","aReported","bReported","CapMin","CapMaxReported","emcoeReport","EAReport");
      //System.out.println(strTemp);
      double dGenMinCapacity=0.0;
      double dGenMaxCapacity=0.0;
      double dGenMaxEA=0.0;
      for(int i = 0; i<supplyOfferByGen.length; i++){
        dGenMinCapacity+=supplyOfferByGen[i][3];
        dGenMaxCapacity+=supplyOfferByGen[i][4];
        dGenMaxEA+=supplyOfferByGen[i][6];
      ...
      //System.out.println("The max total gen Emssion Allowance is: "+dGenMaxEA);
      //System.out.println();
...
/* Temperory remove output -----------    
 ...
 -------------------------------------------------*/      
  ...
  public void post(double[][] dcomm, double[][] dlmp, double[][] dps, int flag,double [][] emtrade){
    DoubleMatrix2D dailyCommitment = new DenseDoubleMatrix2D(dcomm);
    DoubleMatrix2D dayAheadLMP = new DenseDoubleMatrix2D(dlmp);
    //DoubleMatrix2D DailyEmissionBuyingPrice= new DenseDoubleMatrix2D(pb) ;
    //DoubleMatrix2D DailyEmissionSellingPrice= new DenseDoubleMatrix2D(ps);
    DoubleMatrix2D DailyEmissionTradeAmount= new DenseDoubleMatrix2D(emtrade);
    ...
    for(int i=0; i<numGenAgents; i++){
      GenAgent gen = (GenAgent) genAgentList.get(i);
      gen.setCommitment(dailyCommitment.viewColumn(i).toArray());
      gen.setEmTrade(DailyEmissionTradeAmount.viewColumn(i).toArray());
      int k = gen.getAtNode()-1;
      gen.setDayAheadLMP(dayAheadLMP.viewColumn(k).toArray());
